import {
  useMutation,
  useQueryClient,
} from '@tanstack/react-query'
import { zodResolver } from '@hookform/resolvers/zod'
import { useForm } from 'react-hook-form'
import { addStudentWithUserSchema } from '../student/Schemas';
import { studentFetcher } from './fetcher'
import type { AddStudentModel, AddStudentWithUser, StudentModel, StudentWithUser } from '../student/Schemas';
import type { Filters } from '../../teacher/types/apiTypes';
import { generateRandomEmail } from '@/utils/GenerateRandom';


// add student


export function useAddStudent() {

  const queryClient = useQueryClient()
  const { mutate: addStudent } = useMutation({
    mutationFn: studentFetcher.addStudent,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['students'] })
    },
    onMutate: (student) => {
      queryClient.cancelQueries({ queryKey: ['students'] })
      const oldStudentsList = queryClient.getQueryData<Array<StudentWithUser>>([
        'students',
      ])
      const newStudentsList = [...(oldStudentsList ?? []), student]
      queryClient.setQueryData(['students'], newStudentsList)
    },
  })

  const studentForm = useForm<AddStudentWithUser>({
    resolver: zodResolver(addStudentWithUserSchema),
  }
  )

  function onSubmit(data: AddStudentWithUser) {
    addStudent(data)
  }
  return { studentForm, onSubmit }
}


// export const addStudentQueryOptions = () => {
//   const queryClient = useQueryClient()
//   const { mutate: addStudent } = useMutation({
//     mutationFn: studentFetcher.addStudent,
//     onSuccess: () => {
//       queryClient.invalidateQueries({ queryKey: ['students'] })
//     },
//     onMutate: (student) => {
//       queryClient.cancelQueries({ queryKey: ['students'] })
//       const oldStudentsList = queryClient.getQueryData<Array<StudentModel>>([
//         'students',
//       ])
//       const newStudentsList = [...(oldStudentsList ?? []), student]
//       queryClient.setQueryData(['students'], newStudentsList)
//     },
//   })

//   const studentForm = useForm<AddStudentWithUser>({
//     resolver: zodResolver(addStudentWithUserSchema),
//   })

//   function onSubmit(data: AddStudentModel) {
//     const newData = {
//       ...data,
//     }
//     console.log(newData)
//     addStudent(newData)
//   }
//   return { studentForm, onSubmit }
// }


export const getStudentQueryOptions = (studentId: string) => ({
  queryKey: ['student', studentId],
  queryFn: async () => {
    try {
      const response = await studentFetcher.getStudent(studentId)
      console.log(response)
      return response.success && response.data
    }
    catch (error: any) {
      if (error.response?.status === 404) {
        throw new Error('Student not found')
      } else {
        throw new Error('Failed to fetch student data')
      }
    }
  },
})

// get student by id 
// export function useGetStudent(id: string) {
//   return useQuery<StudentModel | null>(getStudentQueryOptions(id))
// }

export const getStudentsQueryOptions = ({
  page,
  size,
  search,
  status,
  grade,
  // email,
  sortBy,
  sortOrder,
}: Partial<Filters<StudentWithUser>>) => ({
  queryKey: ['students', page, size, search, status, grade, sortBy, sortOrder],
  // email  should be added in the search query to filter students by email 
  queryFn: async () => {
    const response = await studentFetcher.getStudents({
      page,
      size,
      search,
      status,
      grade,
      // email,
      sortBy,
      sortOrder,
    })
    console.log("API RESPONSE:", response);
    if (response.success) {
      return {
        data: response.data,
        pagination: response.pagination,
      }
    } else {
      throw new Error(response.errorType)
    }

  },
})



// get student list 
// export function useGetStudents({
//   page,
//   search,
//   size,
//   status,
//   grade,
//   email,
//   sortBy,
//   sortOrder,
// }: Partial<Filters<StudentModel>>) {
//   return useQuery({
//     ...getStudentsQueryOptions({
//       page,
//       search,
//       size,
//       status,
//       grade,
//       email,
//       sortBy,
//       sortOrder,
//     }),
//     placeholderData: keepPreviousData,
//   })
// }



// Edit Student hook should be implemented in the right way after this 


// export const editStudentQueryOptions = (editedStudent: StudentModel) => {
//   const queryClient = useQueryClient();
//   const { mutate: editStudent } = useMutation({
//     mutationFn: studentFetcher.editStudent,
//     onMutate: () => {
//       queryClient.cancelQueries({ queryKey: ["students"] });
//       const oldStudentList = queryClient.getQueryData<Array<StudentModel>>(["students"]);
//       const newStudentList = oldStudentList?.map((student) => student.id === editedStudent.id ? { ...student, ...editedStudent } : student)
//       queryClient.setQueryData(['students'], newStudentList)
//     },
//     onSuccess: () => {
//       queryClient.invalidateQueries({ queryKey: ["students"] })
//     }
//   });


//   function onSubmit(data: ) {
//     const newData: StudentModel = {
//       ...editedStudent,
//       ...data,
//       name: data.name,
//       email: data.email,
//       grade: data.grade,
//       classe: data.classe,
//       parentPhoneNumber: data.parentPhoneNumber,
//       parentName: data.parentName,
//       gender: data.gender,
//       address: data.address,
//       dateOfBirth: data.dateOfBirth,
//       imgSrc: "image/jpeg;base64,",
//       status: editedStudent.status,
//       id: editedStudent.id,
//       userId: editedStudent.userId,
//       schoolId: editedStudent.schoolId,
//     }
//     console.log(newData)
//     editStudent(newData)
//   }

//   const studentForm = useForm<EditStudentModel>({
//     defaultValues: { ...editedStudent },
//     resolver: zodResolver(EditStudentSchema),
//   })

//   return { studentForm, onSubmit }

// }

// export function useEditStudent(editedStudent: StudentModel) {
//   const queryClient = useQueryClient();
//   const { mutate: editStudent } = useMutation({
//     mutationFn: studentFetcher.editStudent,
//     onMutate: () => {
//       queryClient.cancelQueries({ queryKey: ["students", editedStudent.id] });
//       const oldStudentList = queryClient.getQueryData<Array<StudentModel>>(["students", editedStudent.id]);
//       const newStudentList = oldStudentList?.map((student) => student.id === editedStudent.id ? { ...student, ...editedStudent } : student)
//       queryClient.setQueryData(['students', editedStudent.id], newStudentList)
//     },
//     onSuccess: () => {
//       queryClient.invalidateQueries({ queryKey: ["students", editedStudent.id] })
//     }
//   });


//   function onSubmit(data: EditStudentModel) {
//     const newData: StudentModel = {
//       ...data,
//       name: data.name,
//       email: data.email,
//       grade: data.grade,
//       classe: data.classe,
//       parentPhoneNumber: data.parentPhoneNumber,
//       parentName: data.parentName,
//       gender: data.gender,
//       address: data.address,
//       dateOfBirth: data.dateOfBirth,
//       imgSrc: "image/jpeg;base64,",
//       status: editedStudent.status,
//       id: editedStudent.id,
//       userId: editedStudent.userId,
//       schoolId: editedStudent.schoolId,
//     }
//     console.log(newData)
//     editStudent(newData)
//   }

//   const studentForm = useForm<EditStudentModel>({
//     defaultValues: { ...editedStudent },
//     resolver: zodResolver(EditStudentSchema),
//   })

//   return { studentForm, onSubmit }

// }



// delete student
export function useDeleteStudent(id: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: () => studentFetcher.deleteStudent(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['students'] })
    },
    onMutate: () => {
      queryClient.cancelQueries({ queryKey: ['students'] })
      const oldStudentsList = queryClient.getQueryData<Array<StudentModel>>([
        'students',
      ])
      const newStudentsList = oldStudentsList?.filter(
        (old: StudentModel) => old.id !== id,
      )
      queryClient.setQueryData<Array<StudentModel>>(
        ['students'],
        newStudentsList,
      )
    },
  })
}
