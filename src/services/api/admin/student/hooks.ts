...
export function useGetStudent(id: string) {
  return useQuery<StudentWithUser | null>(getStudentQueryOptions(id))
}
...
// export function useEditStudent(editedStudent: StudentWithUser) {
//   const queryClient = useQueryClient()
//   const { mutate: editStudent } = useMutation({
//     mutationFn: studentFetcher.editStudent,
//     onMutate: () => {
//       queryClient.cancelQueries({ queryKey: ['students'] })
//       const oldStudentList = queryClient.getQueryData<Array<StudentWithUser>>([
//         'students',
//       ])
  ...
  return useMutation({
    ...
    onMutate: () => {
      ...
      const newStudentsList = oldStudentsList?.filter(
        (old: StudentWithUser) => old.id !== id,
      )
      queryClient.setQueryData<Array<StudentWithUser>>(
        ['students'],
        newStudentsList,
      )
    },
